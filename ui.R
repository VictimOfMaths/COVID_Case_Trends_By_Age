...
sidebarPanel(
  ...
  radioButtons('plot', 'Age groups', choices=c("Overall", "Age-specific"), selected="Overall", inline=TRUE),
  sliderInput('FitRange', 'Select data range to fit baseline from', min=min(data.cases$date), 
              max=max(data.cases$date), value=c(as.Date("2021-01-08"), as.Date("2021-01-14"))),
